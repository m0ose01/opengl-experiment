...
void main()
{
	...
	float epsilon = light.cutoff_costheta - light.outercutoff_costheta;
	float intensity = clamp(
		(theta - light.outercutoff_costheta) / epsilon,
		0.0,
		1.0
	);
	...
	vec3 diffuse = intensity * diff * light.diffuse * vec3(texture(material.diffuse, TexCoords));
